from .s2s_download import RealTimeGribFileConfig, Param, ReforecastGrib_OnTheFly_FileConfig
import os
...
class S2SOnTheFlyReforecastBase:
    """
    读取S2S数据的回算资料OnTheFly模式
    在该类中，所有关于要素的查询使用以下方式：
        字符串：parameter + level
        例如：
            ①单层要素：'mn2t6', 'mx2t6', '2t', 'tp', '10u', '10v', 'msl', 'sp', 'wtmp'
    ...
    def __init__(self, data_center: Param.data_center, ):
        ...
        assert data_center in ReforecastGrib_OnTheFly_FileConfig.data_center, f'{data_center}不在{Param.data_center}内'

    @property
    def ensNums(self) -> int:
        return ReforecastGrib_OnTheFly_FileConfig.pfEnsNums[self.__data_center] + 1  # 扰动成员数 + 控制成员数

    @property
    def data_center(self) -> str:
        return self.__data_center  # 字符串是不可变类型，安全

    def cal_date_range(self, *parameter_levels, number: int = 0) -> List[str]:
        ans = {}
        for par_ll in parameter_levels:
            parameter, level = Param.separate_FactorLevel(par_ll)
            _period = ReforecastGrib_OnTheFly_FileConfig.init_date_range(data_center=self.__data_center,
                                                                         parameter=parameter,
                                                                         number=number,
                                                                         level=level)
            ans = ans.intersection(set(_period)) if ans != {} else set(_period)
    ...
    def open_source(self, parameter_level: str, cal_date: str, number: int = 0) -> xr.DataArray:
        parameter, level = Param.separate_FactorLevel(parameter_level)
        grib_path = ReforecastGrib_OnTheFly_FileConfig.factor_filepath(data_center=self.__data_center,
                                                                       init_date=cal_date,
                                                                       number=number,
                                                                       parameter=parameter,
                                                                       level=level)
    ...
    def load(self, parameter_level: str, cal_date: str, number: int = 0) -> xr.DataArray:
        data = self.open_source(parameter_level=parameter_level,
                                cal_date=cal_date,
                                number=number)
        if data is not None:
