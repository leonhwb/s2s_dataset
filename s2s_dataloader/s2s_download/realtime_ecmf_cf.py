...
class Download_ECMF_CF_Realtime(RealTimeGribDownload):
    """
    下载ecmf的近实时预报数据，只下载控制模式cf
    """
    ...
    def __init__(self):
        RealTimeGribDownload.__init__(self, number=0, data_center='ecmf')
...
class _Test:
    def test1(self):
        ...
        for f in [f for f in os.listdir(dir) if f.endswith('grib')]:
            ...
            print(f.split('.')[0], data.name)


if __name__ == "__main__":
    print(Download_ECMF_CF_Realtime().download(init_date='20210701', parameter='2t', no_cover=False))
